...
nixlLibfabricTopology::nixlLibfabricTopology()
    : num_gpus(0),
      ...
      discovered_device_type(DeviceType::NONE),
      hwloc_topology(nullptr) {

    NIXL_TRACE << "Starting automatic topology discovery";

    // Attempt topology discovery - throw on any failure
    nixl_status_t status = discoverTopology();
    if (status == NIXL_SUCCESS) {
        NIXL_TRACE << "Topology discovery completed successfully";
        printTopologyInfo();
    } else {
        // Binary failure: either topology discovery succeeds or backend fails
        throw std::runtime_error("Topology discovery failed - no suitable network providers found");
    }
}
...
nixlLibfabricTopology::discoverTopology() {
    ...
    if (discovered_device_type == DeviceType::EFA_DIRECT ||
        discovered_device_type == DeviceType::EFA) {
        ...
        NIXL_INFO << "Using simplified topology for " << fabric_name
        ...
        // For TCP devices, all devices are available to all consumers - no mapping needed
...
nixlLibfabricTopology::discoverEfaDevices() {
    // Try multi-provider discovery with graceful fallback
    auto network_device = LibfabricUtils::getAvailableNetworkDevices();
    fabric_name = network_device.first;
    all_devices = network_device.second;

    num_devices = all_devices.size();

    // Set device type based on discovered provider
    if (fabric_name == "efa-direct") {
        discovered_device_type = DeviceType::EFA_DIRECT;
        NIXL_INFO << "Discovered " << num_devices << " EFA-Direct devices";
    } else if (fabric_name == "efa") {
        discovered_device_type = DeviceType::EFA;
        NIXL_INFO << "Discovered " << num_devices << " EFA devices";
    } else if (fabric_name == "tcp") {
        discovered_device_type = DeviceType::TCP;
        NIXL_INFO << "Discovered " << num_devices << " TCP devices (EFA fallback)";
    } else if (fabric_name == "sockets") {
        discovered_device_type = DeviceType::SOCKETS;
        NIXL_INFO << "Discovered " << num_devices << " socket devices (TCP fallback)";
    } else if (fabric_name == "none" || all_devices.empty()) {
        discovered_device_type = DeviceType::NONE;
...
nixlLibfabricTopology::buildPcieToLibfabricMapping() {
    ...
    // Use the same provider as discovered by getAvailableNetworkDevices()
    // This ensures consistency between device discovery and PCIe mapping
    std::string provider_to_use = fabric_name;
    ...
    hints->fabric_attr->prov_name = strdup(provider_to_use.c_str());

    int ret = fi_getinfo(FI_VERSION(1, 9), NULL, NULL, 0, hints, &info);
    if (ret) {
        NIXL_ERROR << "fi_getinfo failed for PCIe mapping with provider " << provider_to_use << ": "
                   << fi_strerror(-ret);
        fi_freeinfo(hints);
        return NIXL_ERR_BACKEND;
    }
    ...
    NIXL_TRACE << "Built PCIe to Libfabric mapping for " << pcie_to_libfabric_map.size()
               << " devices using provider " << provider_to_use;
    return NIXL_SUCCESS;
}
